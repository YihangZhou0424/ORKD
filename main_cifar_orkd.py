...
from mcl.relation import RKDLoss
...
parser.add_argument('--beta', type=float, default=0.02, help='weight balance for other losses')
parser.add_argument('--gamma', type=float, default=10000, help='weight for classification')
...
def train(epoch, criterion_list, optimizer):
    ...
    criterion_rkd = criterion_list[3]
    ...
    for batch_idx, (input, target, index, contrast_idx) in enumerate(trainloader):
        ...
        loss_rkd = criterion_rkd(embedding)
        loss = loss_cls + args.alpha * loss_div + args.beta * loss_kd + args.gamma * loss_rkd
        loss.backward()
        optimizer.step()
